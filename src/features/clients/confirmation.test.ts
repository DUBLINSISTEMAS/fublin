...
describe("buildConfirmationMessage", () => {
  it("writes the in-store confirmation in the owner's format", () => {
    const text = buildConfirmationMessage({ consultantName: "Anderson", clientName: "Ana Paula Souza", attendance: "presencial", when, interest: "imovel", interestNotes: "apartamento na zona sul", leaderName: "Carlos" });
    expect(text).toBe(
      [
        "*Agendamento*",
        "Presencial",
  ...
  it("says Online, skips the leader line when there is none, and uses the custom interest as the observation", () => {
    const text = buildConfirmationMessage({ consultantName: "Anderson", clientName: "Bia", attendance: "online", when, interest: "outro", interestNotes: "Investimento", leaderName: null });
  ...
  it("falls back to the interest label when there is no detail", () => {
    expect(buildConfirmationMessage({ consultantName: "Anderson", clientName: "Bia", attendance: "presencial", when, interest: "reforma" })).toContain("Observação: Reforma");
  });
});
