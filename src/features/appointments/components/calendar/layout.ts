...
/**
 * Posiciona blocos numa coluna de dia. Eventos que se sobrepõem dividem a largura
 * em "faixas" (como no Google Agenda), do mais cedo para o mais tarde.
 */
export function layoutEvents<T extends Timed>(items: T[]): Positioned<T>[] {
  ...
  let cluster: { start: number; end: number; laneEnds: number[]; members: Positioned<T>[] } | null = null;

  const flush = () => {
    if (!cluster) return;
    const lanes = cluster.laneEnds.length;
    for (const m of cluster.members) m.lanes = lanes;
  ...
  for (const item of sorted) {
    ...
    if (!cluster) cluster = { start: startMin, end: endMin, laneEnds: [], members: [] };
    ...
    cluster.members.push({ item, top: startMin * PX_PER_MINUTE, height: Math.max((endMin - startMin) * PX_PER_MINUTE, MIN_BLOCK_PX), lane, lanes: 1 });
  }
  flush();
  return out;
}
