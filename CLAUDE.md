...
- **Comissão**: `settings.commission.ratePercent` é o padrão; cada venda pode ter a própria taxa em `clients.commission_rate_percent` (null = padrão), escolhida no `<CommissionRateField>` (padrão · 0,5 · 0,6 · 0,8 · personalizar) do cadastro/edição do cliente e do card "Aprovação"; o card do funil mostra "Comissão 0,5%" quando a venda tem taxa própria. Qualquer número de "recebimento" passa por `dealsCommissionCents(deals, taxaPadrão)` (`goals/commission.ts`), que soma carta por carta — nunca `taxa × soma das cartas`.
...
- **Encontros numerados**: `features/appointments/sequence.ts` é a fonte única de "1ª, 2ª, 3ª visita". Só `ATTENDANCE_KINDS` (visita e reunião) contam — ligação e retorno são contatos; cancelado libera o número de volta, quem faltou continua contando. `listClients` devolve `meetingsCount` (feitos) e `meetingsTotal` (marcados), e `nextAppointment.meetingNumber`; as queries da agenda anexam `meetingNumber` a cada linha.
- **Quando o cliente vem**: card do funil, lista e agenda mostram `formatSchedule` (`lib/dates.ts`) — "Hoje · 14:30" ou "Qua, 27 ago · 14:30", nunca só a data. O ícone sai de `APPOINTMENT_KIND_ICON` (loja, online, ligação, retorno) e hoje/amanhã (`isSoon`) ganham fundo escuro.
