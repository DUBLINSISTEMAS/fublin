...
- **Comissão**: `settings.commission.ratePercent` é o padrão; cada venda pode ter a própria taxa em `clients.commission_rate_percent` (null = padrão), editada no card "Aprovação" da página do cliente. Qualquer número de "recebimento" passa por `dealsCommissionCents(deals, taxaPadrão)` (`goals/commission.ts`), que soma carta por carta — nunca `taxa × soma das cartas`.
